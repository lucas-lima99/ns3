...
#include "LoRaBandit_v3.cpp"

int NBanditsA = 4, NBanditsP = 3, WindowSize = 20; //number of actions -- bandits to test
LoRaBandit LoRaMAB(NBanditsA, NBanditsP, WindowSize);
...
void SetTxPower(LoraMacHelper macHelper, LoraHelper helper, NodeContainer& endDevices, uint8_t txPower){
	macHelper.SetTransmissionPower(txPower);
	helper.SetTransmissionPower(txPower);

  //set Tx power for each end device
  for (uint32_t i = 0; i < endDevices.GetN(); ++i) {
      ...
      for (uint32_t j = 0; j < node->GetNDevices(); ++j) {
          ...
          if (loraNetDevice) {
              ...
              if (mac) {
                  mac->SetTransmissionPower(txPower);
                  std::cout << "Potencia de transmissao para o dispositivo " << i << ": " << static_cast<unsigned int>(txPower) << " dBm\n";
...
void LoRaBanditRun(NodeContainer endDevicescows, NodeContainer gateways, Ptr<LoraChannel> channel,double targetRealocation, LoraMacHelper macHelper, LoraHelper helper){ //preciso criar um contexto para buscar as variaveis (endDevicescows, gateways, channel)
	//std::cout << "LoRaBandit Running..." << std::endl;
	LoRaMAB.Run();
	//int A, Power = LoRaMAB.GetMax();
    std::pair<int, int> bestAction = LoRaMAB.GetMax();
    int A = bestAction.first;
    int TxPower = bestAction.second;
	//PrintChosenAction(Action+1);
	macHelper.SetSpreadingFactorsUp (endDevicescows, gateways, channel, A, targetRealocation);
    SetTxPower(macHelper, helper, endDevicescows, TxPower);
}
