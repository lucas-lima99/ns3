...
void LoRaBanditRun(NodeContainer endDevices, NodeContainer gateways, Ptr<LoraChannel> channel, int txPowerdBm, LoraMacHelper macHelper){ //preciso criar um contexto para buscar as variaveis (endDevicescows, gateways, channel)
...
for(int i=10; i < simulationTime; i += MabInterval){
   		Simulator::Schedule (Seconds(i), &LoRaBanditRun, endDevices, gateways, channel, txPowerdBm, macHelper);
