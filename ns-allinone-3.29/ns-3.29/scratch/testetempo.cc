...
// void callprintPerformance (LoraHelper helper, double seconds_init, double seconds_final, std::string filename){

//   std::cout << "chamou print performance" << std::endl;
//   const char * c = filename.c_str ();
//   std::ofstream myfile;
//   myfile.open (filename.c_str(), std::ofstream::app);
//   helper.PrintPerformance (Seconds(seconds_init), Seconds(seconds_final));
//   helper.EnablePacketTracking (filename);
//   helper.PrintPerformance (Seconds(seconds_init), Seconds(seconds_final));
...
//   phyHelper.SetChannel (channelNLOS);
  ...
  // std::cout << "tamanho do vetor channel LOS phy antes do phyhelper nlos: " << channelLOS->GetNDevices() << std::endl; 
  // phyHelper.SetChannel (channelNLOS);
  
  // std::cout << "após o phy helper, tamanho do vetor channel NLOS phy: " << channelNLOS->GetNDevices() << std::endl; 
...
//         // parte para simulação do printperformance
//         // phy->SetChannel(channelLOS); // Canal A candidato
//         // channelLOS->Add(phy);

//       //  Ptr<LoraChannel> testechannel = phy->GetChannel();
//       //   std::cout << "VALOR DE phy: " << phy << std::endl;
//       //   std::cout << "VALOR DE channel LOS: " << channelLOS << std::endl;
//       //   std::cout << "VALOR DE channel dentro do phy: " << testechannel<< std::endl;
...
//         //     Ptr<LoraChannel> testechannel = phy->GetChannel();
//         //     std::cout << "channel NLOS: " << testechannel << std::endl;
...
//         //     // channelLOS->Add(phy);
//         //     // std::cout << "tamanho do vetor channelLOS phy: " << channelLOS->GetNDevices() << std::endl; 
//         //     Ptr<LoraChannel> testechannel = phy->GetChannel();
//         //     std::cout << "channel LOS: " << testechannel << std::endl;
//         //     // channel->Add(phy);

//         // }
//     }

    // std::cout << "VALOR DE channelGetNDevices: " << channelLOS->GetNDevices() << std::endl;

    // for(int i = 0; i < channelLOS->GetNDevices(); i++, j++){
    //     Ptr<NetDevice> channellist = channelLOS->GetDevice(i);
    //     // std::cout << "VALOR DE channellist: " << channellist << std::endl;
    //     Ptr<NetDevice> dstNetDevice = channelLOS->GetDevice (j);
...
// int validacaoLOS = 0, validacaoNLOS = 0;

//   double proporcaoNLOS = 0.5;
//   int endNLOS = proporcaoNLOS * nDevices;
//   int endLOS = nDevices - endNLOS;
//         std::cout << "endNLOS: " << endNLOS << std::endl;
...
//sempre que um pacote é recebido, ele é adicionado à lista Rx
    ...
    // std::cout << "depois do código do gabriel tamanho do vetor channel NLOS phy: " << channelNLOS->GetNDevices() << std::endl; 
...
// 	}
  ...
  Simulator::Schedule(Seconds(simulationTime / 2.0), &ChangeStateToNLOS, endDevices, channelNLOS, gateways, phyHelper, macHelper, helper);
  Simulator::Schedule(Seconds(simulationTime), &callPrintPerformance, &helper, Seconds(0), Seconds(simulationTime));

//   int aux = 0;
//   for(int i= 1 + simulationTime/2; i <= simulationTime + 1; i += MabInterval){
...
// std::ifstream inFile("DadosMABtempo.txt");
//     if (!inFile) {
