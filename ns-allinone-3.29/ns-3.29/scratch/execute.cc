 ...
 */
 ...
 for (u_int32_t i = 0; i < endDevices.GetN(); ++i)
   ...
   // std::cout << "NLOS: " << validacaoNLOS << " LOS: " << validacaoLOS << std::endl;
   // if (testechannel){
   //   std::cout << "canal não nulo" << std::endl;
   //     if(testechannel == channel){
     
   //       std::cout << "você está no caminho, canais iguais" << std::endl;
   //     } else{
   //       std::cout << "canais diferentes" << std::endl;
   //     }
   //   }
   // else{
   //   std::cout << "canal nulo" << std::endl;
   // }
   ...
   // unsigned long timeSeed = duration.count();  // Tempo atual como semente
 
   //   // Cria um gerador com a semente usando std::seed_seq
   // std::seed_seq seedA{timeSeed};
   // std::default_random_engine generator(seedA);
 
   //   // Define um intervalo de valores
   // std::uniform_int_distribution<int> distribution(1, 9);
 
   //   // Gera o número aleatório
   // algoritmo = distribution(generator);
 
 // std::ifstream inFilealeatorio("Escolhaalgoritmoaleatorio.txt");
 // // Abrir o arquivo de saída no modo append (adicionar conteúdo ao final)
 // std::ofstream outFilealeatorio("Escolhaalgoritmoaleatorio.txt", std::ios::app);//     }
 
 // //     // Escrever valores de seed, radius, algoritmo e txPowerdBm no arquivo de saída
 //     outFilealeatorio << "seed: " << runSeed << "," << "raio: " << radius << "," << "algoritmo: " << algoritmo << "," << "txPowerdBm: " << txPowerdBm;
 //      outFilealeatorio << "\n";
 
 // //     // Fechar os arquivos
 //     inFilealeatorio.close();
 //     outFilealeatorio.close();
 
   macHelper.SetSpreadingFactorsUp (endDevices, gateways, channelNLOS, algoritmo, txPowerdBm);
 
 
     // std::cout << "depois de setspreadingfactor tamanho do vetor channel LOS phy: " << channelLOS->GetNDevices() << std::endl; 
 
   // bool multichannel = true;
   // macHelper.SetSpreadingFactorsUp (endDevices, gateways, channelLOS, channelNLOS, multichannel, algoritmo, txPowerdBm);
   ...
   //     }
   // }
     // std::cout << "depois de mudar a potência de transmissão tamanho do vetor channel LOS phy: " << channelLOS->GetNDevices() << std::endl; 
 ...
 //         Ptr<Node> node = endDevices.Get(i);
 //       std::cout << "chegou no node : " << std::endl;
 
 //         Ptr<LoraNetDevice> loraNetDevice = node->GetDevice(0)->GetObject<LoraNetDevice>();
 //       std::cout << "chegou no loranetdevice : " << std::endl;
 
 //         Ptr<LoraPhy> phy = loraNetDevice->GetPhy();
 
 //       std::cout << "chegou no phy : " << std::endl;
 
 //         if (i < endNLOS)
 //         {
 //             phy->SetChannel(channelNLOS); // Canal A
 ...
 //     }
   ...
   // uint32_t nDevicesNLOS = nDevices;  
 ...
 //sempre que um pacote é recebido, ele é adicionado à lista Rx
   ...
   // {
   // 	Ptr<EnergySourceContainer> energySource = CreateObject<EnergySourceContainer>();
   //   // (*j)->AggregateObject(energySource);
   //   // Ptr<EnergySource> energySourceptr = energySource->Get(j);
   //   double new_energy = energySource->Get(i) -> TraceConnectWithoutContext("RemainingEnergy", MakeCallback(&EnergyTraceCallback));
   //   energySource->TraceConnectWithoutContext("RemainingEnergy", MakeCallback(&EnergyTraceCallback));
   //   std::cout<< "energySource " << energySource << " new energy " << new_energy << std::endl;
     
   // }
   
     // std::cout << "depois do código do gabriel tamanho do vetor channel LOS phy: " << channelLOS->GetNDevices() << std::endl;
     // std::cout << "depois do código do gabriel tamanho do vetor channel NLOS phy: " << channelNLOS->GetNDevices() << std::end
 ...
 // }
   ...
   **************/
   ...
   // Simulator::Schedule (Seconds(0), &ChangeState, endDevices, channelLOS, gateways, phyHelper, macHelper, helper);
   // Simulator::Schedule (Seconds(0), &ChangeStatus, algoritmo, txPowerdBm, endDevices, channelLOS, gateways, macHelper);
   // Simulator::Schedule (Seconds(0), &ChangeStatus, algoritmo, txPowerdBm, endDevices, channelNLOS, gateways, macHelper);
   // Simulator::Schedule(Seconds(simulationTime / 2.0), &callPrintPerformance, &helper, Seconds(0), Seconds(simulationTime / 2.0));
 ...
 bool visada;
 std::string local;
 // Simulator::Schedule (Seconds(599), &GetEnergyConsumption, endDevices, sources, appPeriodsSeconds);
  ...
  //        // aux = i;
  //  }
  //  Simulator::Schedule (Seconds(simulationTime), &SaveMABmetrics, chFilenameMAB, local);
 ...
 for(int i=simulationTime/2; i < simulationTime; i += MabInterval){
   ...
   // std::cout << "Energia final após MAB: " << std::fixed << std::setprecision(6) << batteryEnergyFinal << std::endl;
...
//  std::ifstream inFile(fileMAB+".txt");
//      if (!inFile) {
//          std::cerr << "Erro ao abrir o arquivo" << fileMAB << std::endl;
//          return 1;
//      }
...
//      // Abrir o arquivo de saída no modo append (adicionar conteúdo ao final)
//      std::ofstream outFile(filefinal+".txt", std::ios::app);
//      if (!outFile) {
...
//      std::ifstream checkFile(filefinal+".txt");
...
//              outFile << ",N" << (i + 1);
//          }
//          outFile << ",batteryEnergyFinalposMAB,currentposMAB";
//          outFile << "\n";
 
//      }
 
//      // Escrever valores de seed, radius, Q e N
//      outFile << runSeed << "," << radius;
...
//          outFile << "," << n;
//      }
//      outFile << "," << batteryEnergyFinal << "," << current;
//      outFile << "\n";
...
//      std::cout << "Dados com seed e radius adicionados em " << filefinal << ".txt com sucesso!" << std::endl;
